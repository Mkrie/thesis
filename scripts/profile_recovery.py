...
class ProfileRec:
    ...
    @staticmethod
    def solution_at_certain_maximum(xi: tuple[float],
                                    n: int,
    ...
    def linear_programming(self,
                           n_max: int,
                           sigma: float,
                           dir_meas_obj=None) -> tuple[Any]:
        """Solving a linear programming problem."""
        if dir_meas_obj is None:
            dir_meas = DirectMeasurements(profile=self.profile,
                                          sigma=sigma,
                                          dataset_name=self.dataset_name,
                                          dir_profiles_name=self.profile_path)
            dir_meas_obj: tuple[tuple[float, float], ...] = (
                dir_meas.direct_measurements_for_profile())
        last: float = dir_meas_obj[-1][1]
        dir_meas_obj: tuple[tuple[float, float], ...] = tuple((x[0], x[1] - last) for x in dir_meas_obj)
        xi: tuple[float] = tuple(map(lambda x: x[1], dir_meas_obj))
        n: int = len(xi)
        make_data_out = self.make_data(n)
        heights: tuple[float] = make_data_out[0]
        matrix_a: tuple[tuple[float], ...] = make_data_out[1]
        obj: list[int] = tuple([1, *[0]*n])
        bnd: tuple[tuple[int, Any]] = tuple([(0, float("inf")) for _ in range(n + 1)])
        z_min: float = 10**100
        if n_max == 1:
            k = [0]
        elif n_max == 2:
        ...
        opt_optimal: scipy.optimize.OptimizeResult = (
            self.solution_at_certain_maximum(xi=xi,
                                             n=n,
                                             matrix_a=matrix_a,
                                             obj=obj,
                                             bnd=bnd,
            ...
            for k_1 in range(1, n):
                for k_2 in range(k_1, n):
                    for k_3 in range(k_2, n):
                        opt = self.solution_at_certain_maximum(xi=xi,
                                                               n=n,
                                                               matrix_a=matrix_a,
                                                               obj=obj,
                                                               bnd=bnd,
    ...
    def make_data(self, n: int) -> tuple[list[float], list[tuple[float]]]:
...
if __name__ == "__main__":
    ...
    prof_recovery = ProfileRec(profile=profile_name,
    ...
    path_to_profiles = Path(Path(__file__).parents[1],
                            "data",
                            "profiles_1",
                            "csv")
